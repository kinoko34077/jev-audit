...
- Changed-only diff context is trimmed when necessary so each file payload remains within the configured batch character limit after path/entry overhead; coverage reflects the actually retained diff context rather than the pre-trim size
...
- `knt setup`
- `knt verify`
- `python -m unittest discover -s tests -v` (123 tests; Windows skips the two
